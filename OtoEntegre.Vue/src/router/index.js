...
import { createRouter, createWebHistory } from 'vue-router' // <-- değiştir
    ...
    children: [
      {
        ...
        component: () =>
          import(
            /* webpackChunkName: "anasayfa" */ '@/views/pages/anasayfa/index.vue'
          ),
      },
      {
        path: '/theme',
...
const router = createRouter({
  history: createWebHistory(import.meta.env.BASE_URL), // <-- hash yerine history
  routes,
  scrollBehavior() {
    return { top: 0 }
  },
...
router.beforeEach((to, from, next) => {
  ...
  // Login ve Register sayfaları token gerektirmez
  if (to.path.startsWith('/pages/login') || to.path.startsWith('/pages/register')) {
    return next()
  }
